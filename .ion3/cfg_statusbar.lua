...
mod_statusbar.create{
    ...
    -- Sometimes the battery status times out and kills everything. It never
    -- works on my machine anyway, so nix it.
    --template="%date | %load | %linuxbatt%%%linuxbatt_state | %{mem_used}M | %iwinfo_ssid: %iwinfo_link | %06netmon_kbsin in / %06netmon_kbsout out %filler%systray",
    --template="%date | %load | %{mem_used}M | %iwinfo_ssid: %iwinfo_link | %06netmon_kbsin in / %06netmon_kbsout out %filler%systray",
    template="%date | %load | %{mem_used}M | %06netmon_kbsin in / %06netmon_kbsout out %filler%systray",
}
